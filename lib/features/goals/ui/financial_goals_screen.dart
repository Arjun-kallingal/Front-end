...
class _FinancialGoalsScreenState extends State<FinancialGoalsScreen> {
    ...
    if (confirm == true) {
      ...
      if (errorMessage == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Goal deleted successfully.", style: TextStyle(fontSize: 15, color: Colors.white, fontWeight: FontWeight.w500)),
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(errorMessage, style: const TextStyle(fontSize: 15, color: Colors.white, fontWeight: FontWeight.w500)),
            backgroundColor: Colors.redAccent,
            behavior: SnackBarBehavior.floating,
          ),
        );
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 12),
          Expanded(
            child: Text("Sync failed: $message", style: const TextStyle(color: Colors.redAccent, fontSize: 13, fontWeight: FontWeight.w500)),
          ),
        ],
      ),
    );
    ...
    return AnnotatedRegion<SystemUiOverlayStyle>(
      ...
      child: Scaffold(
        ...
        floatingActionButton: FloatingActionButton.extended(
          ...
          label: const Text("New Goal", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15, letterSpacing: 0.5)),
        ),
        body: isLoading
                        ...
                        children: [
                            ...
                            Padding(
                              ...
                              child: Column(
                                children: ongoingGoals
                                    .map((goal) => SleekGoalCard(goal: goal, isDark: isDark, onDelete: () => _confirmDelete(goal)))
                                    .toList(),
                              ),
                            ),
                          ],
                          ...
                          if (filteredGoals.isEmpty && !isLoading)
                            const Padding(
                              ...
                              child: Center(child: Text("No goals found. Start planning today!", style: TextStyle(color: Colors.grey, fontSize: 14, fontWeight: FontWeight.w500))),
                            ),
  ...
  Widget _buildSectionHeader(String title, bool isDark) {
    return Padding(
        ...
        child: Text(title, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, letterSpacing: 1.0, color: isDark ? const Color(0xFF8B90A7) : Colors.grey.shade500)));
  }
  ...
  Widget _buildSearchBar(ColorScheme colorScheme, bool isDark) {
    return Container(
      ...
      decoration: BoxDecoration(color: isDark ? colorScheme.surface : Colors.grey.shade200, borderRadius: BorderRadius.circular(16)),
      child: TextField(
        onChanged: _searchGoals,
        style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: colorScheme.onSurface),
        decoration: InputDecoration(
          isDense: true,
          icon: Icon(Icons.search_rounded, size: 20, color: isDark ? Colors.white54 : Colors.grey.shade600),
          hintText: "Search goals...",
          hintStyle: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: isDark ? Colors.white54 : Colors.grey.shade600),
            ...
            child: Stack(
              ...
              children: [
                CustomPaint(
                  ...
                  painter: _DonutPainter(completedPercent: completedPercent, activePercent: activePercent, bgColor: isDark ? Colors.white10 : Colors.grey.shade100, completedColor: const Color(0xFF10B981), activeColor: const Color(0xFF3B82F6)),
                ),
                Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Text(totalGoals.toString(), style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: isDark ? Colors.white : const Color(0xFF0F172A), height: 1.0)),
                    const SizedBox(height: 4),
                    Text("Goals", style: TextStyle(fontSize: 12, color: Colors.grey.shade500, fontWeight: FontWeight.w600)),
                  ],
                ),
              ],
            ),
...
class SleekGoalCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color finalCardBgColor = isDark ? (isCompleted ? const Color(0xFF162A22) : const Color(0xFF152238)) : (isCompleted ? const Color(0xFFECFDF5) : const Color(0xFFEFF6FF));
    final Color finalBorderColor = isDark ? accentColor.withOpacity(0.3) : (isCompleted ? const Color(0xFFA7F3D0) : const Color(0xFFBFDBFE));
    final Color finalWatermarkColor = isDark ? accentColor.withOpacity(0.05) : (isCompleted ? const Color(0xFFD1FAE5) : const Color(0xFFDBEAFE));
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [if (!isDark) BoxShadow(color: accentColor.withOpacity(0.1), blurRadius: 16, offset: const Offset(0, 6))],
      ),
      child: Material(
        ...
        child: InkWell(
          ...
          child: ClipRRect(
            ...
            child: Stack(
              children: [
                Positioned(right: -20, bottom: -20, child: Icon(isCompleted ? Icons.military_tech_rounded : Icons.radar_rounded, size: 140, color: finalWatermarkColor)),
                Padding(
                  padding: const EdgeInsets.all(20),
                  child: Column(
                    children: [
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(goal.title, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 17, color: textPrimary), maxLines: 1, overflow: TextOverflow.ellipsis),
                                const SizedBox(height: 4),
                                Row(
                                  children: [
                                    Icon(Icons.category_rounded, size: 14, color: textSecondary),
                                    const SizedBox(width: 4),
                                    Flexible(child: Text(goal.category, style: TextStyle(fontSize: 13, color: textSecondary, fontWeight: FontWeight.w500), maxLines: 1, overflow: TextOverflow.ellipsis)),
                                    const SizedBox(width: 6),
                                    Text("•", style: TextStyle(color: textTertiary, fontSize: 13)),
                                    const SizedBox(width: 6),
                                    Text(DateFormat('MMM dd, yyyy').format(goal.targetDate), style: TextStyle(fontSize: 13, color: textSecondary, fontWeight: FontWeight.w500)),
                                  ],
                                ),
                              ],
                            ),
                        ...
                        children: [
                          Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 4),
                              Text("₹${goal.currentAmount.toInt()}", style: TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: textPrimary, letterSpacing: -0.5)),
                            ],
                          ),
                          Column(
                            crossAxisAlignment: CrossAxisAlignment.end,
                            children: [
                              ...
                              const SizedBox(height: 4),
                              Text("₹${goal.targetAmount.toInt()}", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: textPrimary)),
                            ],
                          ),
                        ],
                      ),
                        ...
                        children: [
                          Expanded(
                            child: ClipRRect(
                              ...
                              child: LinearProgressIndicator(value: progress, minHeight: 8, backgroundColor: isDark ? Colors.white10 : Colors.black.withOpacity(0.05), valueColor: AlwaysStoppedAnimation(accentColor)),
                            ),
                          ),
                          const SizedBox(width: 14),
                          SizedBox(width: 45, child: Text("${(progress * 100).toInt()}%", textAlign: TextAlign.right, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: accentColor))),
                        ],
                      ),
                      ...
                      Row(
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(color: finalIconBgColor, borderRadius: BorderRadius.circular(8)),
                            child: Text(predictionText, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: isDark ? accentColor : badgeTextColor)),
                          ),
                          if (!isCompleted)
                            Row(
                              children: [
                                ...
                                const SizedBox(width: 4),
                                Text("${daysRemaining > 0 ? daysRemaining : 0} days left", style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: textSecondary)),
                              ],
                            ),
                        ],
                      ),
...
class _DonutPainter extends CustomPainter {
  ...
  _DonutPainter({required this.completedPercent, required this.activePercent, required this.bgColor, required this.completedColor, required this.activeColor});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (completedPercent > 0) {
      final completedPaint = Paint()..color = completedColor..style = PaintingStyle.stroke..strokeWidth = strokeWidth..strokeCap = StrokeCap.round;
    ...
    if (activePercent > 0) {
      ...
      if (sweep > 0) canvas.drawArc(rect, startAngle, sweep, false, activePaint);
    }
  }
