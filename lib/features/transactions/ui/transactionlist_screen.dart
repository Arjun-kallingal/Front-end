...
class _TransactionListScreenState extends State<TransactionListScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final Map<String, dynamic> accountData = await AccountService.getAccountDashboard();
  ...
  Future<void> _fetchMore() async {
    ...
    try {
      ...
      if (selectedAccountName != "All Accounts" && _accounts.isNotEmpty) {
        try {
          resolvedAccountId = _accounts.firstWhere((a) => a.name == selectedAccountName).id;
        } catch (_) {}
      }
  ...
  Future<void> _handleReversal(TransactionModel tx) async {
    ...
    if (confirm == true) {
      ...
      // Calls the backend reversal logic under the hood
      ...
      if (result['success']) {
        ...
        setState(() => _isLoading = false);
        // 🔥 Trigger the Premium Error Dialog instead of a generic snackbar
  ...
  // ── Tile Builder Helper ───────────────────────────────────────────────────
  Widget _buildTransactionTile(TransactionModel tx, ThemeData theme, ColorScheme colorScheme, Color textSec, bool isCash) {
  ...
  Widget _buildTransactionList(ThemeData theme, ColorScheme colorScheme, bool isDark, Color textSec) {
    final list = _transactions.where((tx) {
      ...
      final mCategory = selectedCategory == "All" || tx.category.toLowerCase() == selectedCategory.toLowerCase();
      final mAccount = selectedAccountName == "All Accounts" || tx.accountName == selectedAccountName;

      bool mType;
      switch (selectedType) {
        case "Income": mType = tx.type == "INCOME"; break;
        case "Expense": mType = tx.type == "EXPENSE" && tx.direction != "GOAL_ALLOCATION" && tx.direction != "GOAL_COMPLETION"; break;
        case "Reserved": mType = tx.direction == "GOAL_ALLOCATION"; break;
        case "Transfer": mType = tx.type == "TRANSFER"; break;
        default: mType = true;
      }
      ...
      if (startDate != null && endDate != null) {
        ...
        mDate = txDate.isAfter(start.subtract(const Duration(days: 1))) && txDate.isBefore(end.add(const Duration(days: 1)));
      }
    ...
    if (list.isEmpty) {
      return ListView(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Center(
            child: Text("No transactions found", style: TextStyle(fontSize: 16, color: textSec, fontWeight: FontWeight.w500)),
          ),
        ],
      );
    }
      ...
      itemBuilder: (context, i) {
        ...
        if (i == list.length) {
          if (_isFetchingMore) {
            return Padding(
              ...
              child: Center(child: CircularProgressIndicator(color: colorScheme.secondary, strokeWidth: 2)),
            );
          }
          if (_nextCursor == null && list.isNotEmpty) {
            return Padding(
              padding: const EdgeInsets.all(16),
              child: Center(child: Text("You have reached the end", style: TextStyle(color: textSec, fontSize: 13))),
            );
          }
        ...
        if (showHeader) {
          ...
          if (txDate.year == today.year && txDate.month == today.month && txDate.day == today.day) {
            headerText = "Today";
          } else if (txDate.year == today.year && txDate.month == today.month && txDate.day == today.day - 1) {
          ...
          return Column(
            ...
            children: [
              Padding(
                ...
                child: Text(
                  headerText,
                  style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: textSec),
                ),
              ),
              tileContent,
            ],
      ...
      child: Stack(
        children: [
          ...
          AnimatedContainer(
            ...
            child: Container(
              color: Theme.of(context).scaffoldBackgroundColor, // Ensure drag covers background
              child: widget.child,
            ),
          ),
        ],
